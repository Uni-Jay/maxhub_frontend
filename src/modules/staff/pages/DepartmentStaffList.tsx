...
import { Search, Users, ChevronLeft, ChevronRight, Mail, Phone, Building2 } from 'lucide-react';

// HOD's own dedicated team view — deliberately a separate page from the
// company-wide StaffList used by Admin/Super Admin/HR. The backend already
// scopes GET /staff to her department only, but sharing one component meant
// the page still read "Staff Directory" with company-wide framing and an
// Add Staff button that flickered in/out — confusing for a role that's
// view-only and department-scoped. This page is unambiguously hers.
...
export default function DepartmentStaffList() {
  ...
  const { data, isLoading, isError } = useApiQuery(
    ['staff', 'own-department', { search, page }],
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            ...
            {total > 0 ? `${total} team member${total !== 1 ? 's' : ''}` : 'View your department roster'}
          </p>
        </div>
      </div>
      ...
      {isLoading && !data ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-16 text-center">
          ...
          <p className="font-medium text-gray-900 dark:text-white">No team members found</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {search ? 'Try adjusting your search' : 'No staff have been assigned to your department yet'}
          </p>
        </div>
      ) : (
            ...
            <Link key={s.id} to={`/staff/${s.id}`} className="group">
              <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 hover:shadow-md hover:border-gray-200 dark:hover:border-gray-600 transition-all">
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {(s.position || s.designation?.name) && (
                    <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                      <Building2 className="h-3.5 w-3.5 flex-shrink-0" />
                      <span className="truncate">{s.position || s.designation?.name}</span>
                    </div>
                  )}
                </div>
              </div>
